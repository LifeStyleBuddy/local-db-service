...
public class Person implements Serializable {
    ...
    @OneToMany(mappedBy="person",cascade=CascadeType.ALL,fetch=FetchType.EAGER)
    private List<Goal> goalHistory; // all goals
    ...
    public List<Measure> getCurrentHealth() {
    	if(currentHealth==null)
    		this.setCurrentHealth(refreshLists(this.idPerson));
    ...
    @Transient
    private List<Goal> currentGoal; // one for each type of measure
    
    @XmlElementWrapper(name = "currentGoal")
    @XmlElement(name = "goal")
    public List<Goal> getCurrentGoal() {
    	if(currentGoal==null)
    		this.setCurrentGoal(refreshGoalLists(this.idPerson));
        return currentGoal;
    }
    
    public void setCurrentGoal(List<Goal> cg) {
        this.currentGoal = cg;
    }
	...
	public List<Goal> getGoalHistory() {
		if (this.goalHistory.isEmpty() && !this.getCurrentGoal().isEmpty()){
			this.setGoalHistory(this.getCurrentGoal());
		}
			
		return this.goalHistory;
	}

	public void setGoalHistory(List<Goal> goalHistory) {
    ...
    public static List<Measure> refreshLists(long id){
    	EntityManager em = LifeCoachDao.instance.createEntityManager();
    ...
    public static List<Goal> refreshGoalLists(long id){
    	EntityManager em = LifeCoachDao.instance.createEntityManager();
    	List<Goal> list = em
				.createQuery("SELECT p FROM Goal p WHERE p.person.idPerson = :id ORDER BY p.type", Goal.class)
				.setParameter("id", id).getResultList();
	    LifeCoachDao.instance.closeConnections(em);
	    Map<String, Goal> currentStatus = new HashMap<>();
    	for (int i = 0; i<list.size(); i++){
    		String key = list.get(i).getType();
    		if (!currentStatus.containsKey(key)){
    			currentStatus.put(key, list.get(i));
    	...
    	for (Goal g : currentStatus.values()){
    		l.add(g);
    	}
    	return l;
